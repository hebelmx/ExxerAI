...
**IMPORANT**: Supabase has updated a couple environment variables so you may have to add some new default values in your config that I have in my config.example if you have had this project up and running already and are just pulling new changes. Specifically, you need to add "POOLER_DB_POOL_SIZE=5" to your config. This is required if you have had the package running before June 14th.
...
1. Make a copy of `config.example` and rename it to `config` in the root directory of the project
...
3. Set up A records for your DNS provider to point your subdomains you'll set up in the config file for Caddy
...
3. Create credentials for every service:
   ...
   Postgres (through Supabase): use DB, username, and password from config. IMPORTANT: Host is 'db'
