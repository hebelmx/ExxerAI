...
foreach ($proj in $Projects) {
    ...
    # --- Option 1: Save to file AND show on console
    dotnet restore $FullPath -v:diag 2>&1 | Tee-Object -FilePath "$LogDir\restore-$($proj.Replace('\','_')).log"
# --- Option 2: Save to file ONLY (silent terminal)
# dotnet restore $FullPath -v:diag *> "$LogDir\restore-$($proj.Replace('\','_')).log"
# --- Option 3: Terminal ONLY (no logging)
...
# dotnet restore $FullPath -v:diag 2>&1 | Tee-Object -FilePath "$LogDir\restore-$($proj.Replace('\','_')).log"
    ...
    # --- Option 1: Save to file AND show on console
    dotnet build $FullPath -v:diag 2>&1 | Tee-Object -FilePath "$LogDir\build-$($proj.Replace('\','_')).log"
# --- Option 2: Save to file ONLY (silent terminal)
# dotnet build $FullPath -v:diag *> "$LogDir\build-$($proj.Replace('\','_')).log"
# --- Option 3: Terminal ONLY (no logging)
...
if ($Test) {
    foreach ($proj in $Projects) {
        if ($proj -like "*Tests.csproj") {
            ...
            # --- Option 1: Save to file AND show on console
    dotnet test $TestPath -v:diag 2>&1 | Tee-Object -FilePath "$LogDir\test-$($proj.Replace('\','_')).log"
# --- Option 2: Save to file ONLY (silent terminal)
# dotnet test $TestPath -v:diag *> "$LogDir\test-$($proj.Replace('\','_')).log"
# --- Option 3: Terminal ONLY (no logging)
